...
# ____ Custom CSS for the Cyberpunk SOC aesthetic ____
st.markdown("""
    <style>
...
@st.cache_data(ttl=2) # Cache data for 2 seconds to prevent API spam
...
# Conditional Formatting Function for the Table
...
if raw_log:
    ...
    with col_left:
        ...
        if devices:
            ...
            # Apply our custom glowing highlight to the dataframe!
            styled_df = df.style.apply(highlight_threats, axis=1)
            st.dataframe(styled_df, use_container_width=True, hide_index=True)
            
            # --- NEW: PLOTLY DONUT CHART (CYBERPUNK V3.1 UPGRADE) ---
            st.markdown("### 📊 Hardware Fingerprint Distribution")
            vendor_counts = df['Vendor'].value_counts().reset_index()
            vendor_counts.columns = ['Vendor', 'Count']
            
            # 🚨 HIGH CONTRAST CYBERPUNK PALETTE TEST 🚨
            color_map = {
                "Apple": "#FF00FF",       # Hot Magenta
                "Samsung": "#0088FF",     # Deep Electric Blue
                "Cisco": "#00FF00",       # Matrix Green
                "🚨 Rogue IoT (Raspberry Pi)": "#FF6600", # Warning Neon Orange!
                "Unknown": "#FFFFFF"      # Pure White
            }
            
            fig = px.pie(vendor_counts, names='Vendor', values='Count', hole=0.65, color='Vendor', color_discrete_map=color_map)
            
            # UPGRADE: Thicker borders and MUCH larger internal fonts
            fig.update_traces(
                textposition='inside', 
                textinfo='percent', 
                marker=dict(line=dict(color='#0E1117', width=5)), # Matched streamit dark background
                textfont=dict(size=18, family="Courier New, monospace", color="black"), # <-- FONT SIZE INCREASED
                hoverinfo="label+percent+name"
            )
            
            # UPGRADE: Larger legend font and brighter text color
            fig.update_layout(
                plot_bgcolor='rgba(0,0,0,0)', 
                paper_bgcolor='rgba(0,0,0,0)', 
                font=dict(color='#E2E8F0', family="Courier New, monospace", size=14), # Base font size
                ...
                legend=dict(
                    ...
                    x=0.5,
                    title_text="",
                    font=dict(size=16) # <-- LEGEND FONT SIZE INCREASED
                )
            )
